...
try {
    ...
    if ($response.StatusCode -eq 200) {
        ...
        Write-Host "🔔 Deberías haber recibido la notificación AHORA MISMO" -ForegroundColor Green
        Write-Host ""
        Write-Host "Si NO recibiste notificación, el problema está en:" -ForegroundColor Yellow
        Write-Host "  1. La suscripción de push (revisar push_subscriptions)" -ForegroundColor Yellow
